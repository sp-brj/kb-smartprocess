...
**ВАЖНО:** Задача НЕ считается завершённой, пока результат не проверен от имени пользователя: в браузере через Claude in Chrome (прод после деплоя или локальный dev-сервер) либо через API/MCP. Не говорить «готово» без такой проверки. Push в `main` = деплой на прод, только с явного разрешения.

После завершения любой задачи по изменению кода:

1. **Собрать проект и прогнать юнит-тесты**:
   ```bash
   npm run build
   npm run test:unit
   ```
