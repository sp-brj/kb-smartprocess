  ...
  return (
    <div>
      ...
      {folder.children.length > 0 && (
        <div className="mb-6">
          <h2 className="text-sm font-medium text-muted-foreground mb-3">Подпапки</h2>
          <div className="flex flex-wrap gap-2">
            {folder.children.map((child) => (
              <Link
                key={child.id}
                href={`/folders/${child.slug}`}
                className="inline-flex items-center gap-2 px-3 py-2 bg-card rounded-lg shadow hover:shadow-md transition-shadow"
              >
                <svg
      ...
      {folder.articles.length === 0 ? (
        <div className="bg-card rounded-lg shadow p-8 text-center">
          <p className="text-muted-foreground mb-4">В этой папке пока нет статей</p>
          <Link
            href="/articles/new"
